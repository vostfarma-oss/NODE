// Анимированная сетка на фоне (уменьшенная)
...
function drawGrid() {
    ...
    ctx.strokeStyle = 'rgba(66, 245, 188, 0.05)';
    ctx.lineWidth = 0.6;
    ...
    ctx.fillStyle = 'rgba(66, 245, 188, 0.08)';
    for (let x = 0; x < w; x += step) {
        for (let y = 0; y < h; y += step) {
            ctx.beginPath();
...
const platforms = {
    windows: {
        icon: '🪟',
        name: 'Windows 10/11',
        desc: 'Установщик 12 МБ • Один раз дайте разрешения',
        file: 'Node_Setup.exe',
...
downloadBtn.addEventListener('click', () => {
    const p = platforms[currentPlatform];
    alert(`Скачивание ${p.file}\n\nФайл появится после выхода версии 1.0. Подпишитесь в Telegram.`);
});
